...
echo "ds;e;a;amp;b;gterr;m;rep;ex;ax" > $allout 
...
do
  for e in 0.00 0.01;
  do
  for amp in 0.00 0.05;
  do
  for b in 0.00 0.05 0.10 0.20;
  do

   simdir=$dsroot/sim${sim}.C${cov}.D${a}E${e}A${amp}B$b
...
#    grep -o -h -e 'E{.*}' $simdir/rxmlgt_trees/$model/t*.raxml.bestModel | grep -n '' | sed 's/[:\/]/;/g' | sed 's/[E{}]//g' > $outfile
    ...
    sed "s/^/sim${sim}_C${cov};$e;$a;$amp;$b;$gterr;$model;/" $outfile >> $allout
  done
  done
  done
  done
