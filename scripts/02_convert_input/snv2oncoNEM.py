...
if __name__ == "__main__":
    if len(sys.argv) == 1: 
        print "Usage: ./snv2oncoNEM.py snv_hap.phy true_hap.phy [keep | remove | missing]"
        sys.exit()
    ...
    onconem_file = snv_file + "." + trial_mode + ".onco"
    ...
    onconem_mat = [[0 for x in range(n_taxa)] for y in range(n_snv)] 
    
    with open(onconem_file, "w") as fseq:
	fseq.write("snv ")
        j = 0  
        for name in names:
           seq = snvs[name]
	...
	fseq.write("\n")
        for i in range(n_snv):
            ...
            for j in range(n_taxa):
              fseq.write("%d" % onconem_mat[i][j]);
              if j < n_taxa-1:
                fseq.write(" ") 
            fseq.write("\n") 
    
    print "oncoNEM mutation matrix: ", onconem_file
