...
if __name__ == "__main__":
    if len(sys.argv) == 1: 
        print "Usage: ./snv2sifit.py snv_hap.phy true_hap.phy [keep | remove | missing]"
        sys.exit()
    ...
    sifit_file = snv_file + "." + trial_mode + ".sf"
    ...
    sifit_mat = [[0 for x in range(n_taxa)] for y in range(n_snv)] 
    ...
    with open(sifit_file, "w") as fseq:
        for i in range(n_snv):
            fseq.write("%d " % (i+1))
            for j in range(n_taxa):
              fseq.write("%d" % sifit_mat[i][j]);
              if j < n_taxa-1:
                fseq.write(" ") 
            fseq.write("\n") 
    
    print "Names: ", names_file
    print "SiFit mutation matrix: ", sifit_file
