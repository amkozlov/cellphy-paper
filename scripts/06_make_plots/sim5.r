...
mlvl <- c("tnt_missing", "tnt_missing_rfmin",
          ...
          "GTGTR4+FO", "GTGTR4+FO+E", "GTGTR4+FO+ERR_PT19", "GPGTR4+FO+ERR_P20", "vcf_GTGTR4+FO", "vcf_GPGTR4+FO")
mlbl <- c("TNT", "TNT (min)", "SCITE", "infSCITE", "infSCITE (min)", "SiFit", "SiFit_r10", "SCIPhI", "ScisTree", "SASC",
          "CellPhy-NOERR", "CellPhy-ML10", "CellPhy+PT19", "CellPhy-ML16", "CellPhy-GL10", "CellPhy-GL16")
...
p <- p + facet_grid(all$V3+all$V5+all$V4~all$V6)
#, scales = "free")
#, labeller = label_wrap_gen(30, multi_line=FALSE)
...
p <- p + ggtitle("Tree reconstruction accuracy in Scenario 5: NGS doublets")

p <- p + theme(plot.title=element_text(vjust=2, hjust = 0.5, color="black"))

p <- p + labs(subtitle = "Doublet rate", y = "Phylogenetic accuracy", fill = "Method")

p <- p + guides(fill = guide_legend(nrow = 1))

p <- p + scale_y_continuous(breaks=seq(0,1.0,0.1), sec.axis = dup_axis(name = "", breaks=NULL))
...
#fname <- sprintf("sim6_C%d_rfdist.pdf", cov)

if (showmin == FALSE) {
  fname <- sprintf("SIM5_C%d_rfdist.png", cov)
  ftitle = sprintf("SIM5 C%d RFdist", cov)
} else {
  fname <- sprintf("SIM5_C%d_rfdist_min.png", cov)
  ftitle = sprintf("SIM5 C%d RFdist (MIN)", cov)
}

ggsave(fname, width=24, height=18, units = "cm", title=ftitle)
