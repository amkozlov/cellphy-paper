...
mlvl <- c("onem_keep", "onem_remove", "onem_missing",
          "tnt_keep", "tnt_remove", "tnt_missing",
...
mlbl <- c("oncoNEM (keep)", "oncoNEM (remove)", "oncoNEM",
          "TNT (keep)", "TNT (remove)", "TNT",
...
cbFill <- c('lightgray','palegreen','lightblue', 'khaki', 'indianred', 'red', 'darkred')
...
p <- p + ggtitle(sprintf("Tree reconstruction accuracy in Simulation 1: target-ISM, %d SNVs", j))
...
fname <- sprintf("SIM1j%d_rfdist.png", j)

ftitle = sprintf("SIM2 RFdist %d SNVs", j)

ggsave(fname, width=28, height=23, units = "cm", title=ftitle)
