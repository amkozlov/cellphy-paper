...
all$af <- factor(all$a, levels = alvl, labels = albl)

mlvl <- c("GTM0+FE+E", "GTHKY4+FE+E", "GTHKY4+FC+E", "GTGTR4+FO+E", "GPGTR4+FO+ERR_P20")
mlbl <- c("JC69+E  ", "K80+E  ", "HKY85+E  ", "GTR4+E", "GT16+E")
...
p <- ggplot(data = all, aes(x = af, y = ax * 100)) + geom_boxplot(aes(group=af), outlier.size=1, outlier.color="lightgrey") 

p <- p + geom_point(aes(y=a*100), colour = "red", size = 2) 
...
p <- p + ggtitle("True vs. estimated ADO rate")

p <- p + theme(plot.title=element_text(vjust=2, hjust=0.5))

p <- p + labs(x = "\nTrue ADO rate (%)", y = "Estimated ADO rate (%) \n")
...
p <- p + scale_color_manual(name="True ADO rate", values=cbFill)
...
ggsave("gt_adox.png", width=20, height=27, units = "cm")
